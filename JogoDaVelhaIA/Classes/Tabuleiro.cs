...
{
	public class Tabuleiro:JogoDaVelha
	{
		...
		public bool RealizarJogadaIAV1()
		{

			bool GameOver = FimDeJogo();
			for (int i = 0; i < 3; i++)
			{
				for (int j = 0; j < 3; j++)
				{
					if (TabuleiroJogo[i, j] == EnumTipoPeca.VAZIO)
					{
						InserirJogada(i, j, EnumTipoPeca.O);
						GameOver = FimDeJogo();
						if (GameOver)
						{
						...
						LiberaCelula(i, j);
						InserirJogada(i, j, EnumTipoPeca.X);
						GameOver = FimDeJogo();
						if (GameOver)
						{
							LiberaCelula(i, j);
		...
		public void LiberaCelula(int linha, int coluna)
		{
			TabuleiroJogo[linha, coluna] = EnumTipoPeca.VAZIO;
		}
